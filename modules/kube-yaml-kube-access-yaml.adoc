...
. On the listening site, for example `west` namespace, create an `AccessGrant` resource:
+
[,yaml]
----
apiVersion: skupper.io/v2alpha1
...
. Securely transfer the `token.yaml` file to context of the connecting site.
If you have both sites available from your terminal session, this step is not required.
. On the connecting site, apply the token and check status:
+
[,shell]
----
...
. On the connecting site, check link status:
+
[,shell]
----
kubectl get link
