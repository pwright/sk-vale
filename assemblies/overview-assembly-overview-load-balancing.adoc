...
A xref:kube-creating-multikeylistener-yaml[multi-key listener] provides per-service control over load balancing and failover by binding a single endpoint to multiple routing keys (connectors).
...
For configuration details and examples, see xref:kube-creating-multikeylistener-yaml[Creating a multi-key listener using YAML].
...
For per-service control, use a xref:kube-creating-multikeylistener-yaml[multi-key listener] instead.
...
* xref:kube-link-cost-cli[Specifying link cost using the CLI]
* xref:kube-link-cost-yaml[Specifying link cost using YAML]
